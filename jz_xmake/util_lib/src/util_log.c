...
static void get_timestamp(char *buffer)
{
    ...
    if (1 == g_time_type) {
        ...
        sec = tp.tv_sec % 60;
        tmp_value = tp.tv_sec / 60;
        min = tmp_value % 60;
        tmp_value = tmp_value / 60;
        hour = tmp_value % 24;
        day = tmp_value / 24;
        len = snprintf(buffer, 32, "[%02d %02d:%02d:%02d %03d] ", day, hour,
            min, sec, msec);
    } else {
        time_t t;
        struct tm *p;
...
static int get_curtime(char *outTimeStr)
{
    ...
    sprintf(outTimeStr, "%04d%02d%02d%02d%02d%02d", tmTime->tm_year + 1900,
...
static int file_alives_proc()
{
    ...
    for (i = 0; i < LOGFILE_MAXCOUNT; i++) {
        if (strlen(file_names[i]) > 0) {
            ...
            memcpy(ftime,
                &file_names[i]
                           [strlen(LOGFILE_PREFIX) + 1 + strlen(file_prifix)],
                8);
            printf("file_time=%s\n", ftime);
...
static int recv_msg(void)
{
    ...
    if (rsize == -1) {
        ...
        get_curtime(tmpTime);
        sprintf(tmpfname, "%s/%s%s_%8.8s.log", LOGFILE_PATH, LOGFILE_PREFIX,
            file_prifix, tmpTime);
        fp = fopen(tmpfname, "a");
        if (NULL == fp) {
            ...
            if ((filesize / 1024) > LOGFILE_MAXSIZE) {
                fprintf(stderr,
                    "failed to write log,only allow maxsize=%ukb,current "
                    "size=%lukb\n",
...
void mylog1(
    const char *filename, int line, enum LogLevel level, const char *fmt, ...)
{
    if (level > LOGLEVEL)
        return;
